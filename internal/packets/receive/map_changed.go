...
func (m *MapChanged) Deserialize() error {
    ...
    common.Log(common.LogPacket, common.LogVeryVerbose, "MapChanged - ConnID=%d, SourceIP='%s', mapName='%s', coords=(%d,%d)", m.ConnID, m.SourceIP, mapName, coordX, coordY)

    SetConnectionMap(m.ConnID, mapName)
    SetPendingMapChange(m.SourceIP, mapName, coordX, coordY)
    
    common.Log(common.LogPacket, common.LogInfo, "[%d] Map changed to: %s (X:%d Y:%d) - Pending match for %s", m.ConnID, mapName, coordX, coordY, m.SourceIP)
    return nil
}
